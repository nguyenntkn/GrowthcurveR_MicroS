...
for (i in 1:nrow(pca_gc_out6)) {
  wellID <- toString(pca_gc_out6[i,1]) 
  col1 <- append(col1,get_treatment(wellID)) }

pca_gc_out6 <- pca_gc_out6 %>% mutate(Treatment = col1)

# Perform PCA 
pca6.res <- prcomp(pca_gc_out6 %>% select(k:sigma), 
                  center=TRUE, 
                  scale=TRUE)

pca_gc_out6$Treatment <- factor(pca_gc_out6$Treatment, 
                               levels = c("UNT", 
                                         "Neg ctrl",
                                         "Pos ctrl",
                                         "FA103",
...
as_data_frame(list(PC1=pca6.res$x[,1],
                   PC2=pca6.res$x[,2],
                   Treatments = as.character(pca_gc_out6$Treatment))) %>% 
...
write.xlsx(pca_gc_out6, "/PATH_TO_FILE.xlsx")
